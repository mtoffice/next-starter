...
const GET_PRODUCTS_BY_CATEGORY = gql`
  query ProductsByCategory {
    productsByCategory(category: "oprema za vodenje prometa" ){
      tag
      category
      subcategory
      class
...
class znakiZaOznacevanjeRobaVoziscaInPreprecevanjeVoznje extends React.Component {
  ...
  render() {
    ...
    return (
      <App pathname={"oprema-za-vodenje-prometa"}>
        <Wrapper>
          <Query  query={GET_PRODUCTS_BY_CATEGORY}>
            {({ loading, error, data }) => {
              if(loading){
                return(
                  <React.Fragment>
                    <TitleH1>Oprema za vodenje prometa</TitleH1>
                    <SignsWrapper>
                      <Sign/><Sign/><Sign/><Sign/>
                      <Sign/><Sign/><Sign/><Sign/>
                      <Sign/><Sign/><Sign/><Sign/>
              ...
              return (
                <Signs subcategory2={subcategory} pathname={"oprema-za-vodenje-prometa"} subcategories={subcategories} data={data} />
              );
            }}
          </Query>
        </Wrapper>
