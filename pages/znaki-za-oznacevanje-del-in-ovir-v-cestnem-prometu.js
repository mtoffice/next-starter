...
const GET_PRODUCTS_BY_CATEGORY = gql`
  query ProductsByCategory {
    productsByCategory(category: "znaki za označevanje del in ovir v cestnem prometu" ){
      tag
      category
      subcategory
      class
...
class znakiZaOznacevanjeRobaVoziscaInPreprecevanjeVoznje extends React.Component {
  ...
  render() {
    ...
    return (
      <App pathname={"znaki-za-oznacevanje-del-in-ovir-v-cestnem-prometu"}>
        <Wrapper>
          <Query  query={GET_PRODUCTS_BY_CATEGORY}>
            {({ loading, error, data }) => {
              if(loading){
                return(
                  <React.Fragment>
                    <TitleH1>Znaki za oznacevanje del in ovir v cestnem prometu</TitleH1>
                    <SignsWrapper>
                      <Sign/><Sign/><Sign/><Sign/>
                      <Sign/><Sign/><Sign/><Sign/>
                      <Sign/><Sign/><Sign/><Sign/>
              ...
              return (
                <Signs subcategory2={subcategory} pathname={"znaki-za-oznacevanje-del-in-ovir-v-cestnem-prometu"} subcategories={subcategories} data={data} />
              );
            }}
          </Query>
        </Wrapper>
