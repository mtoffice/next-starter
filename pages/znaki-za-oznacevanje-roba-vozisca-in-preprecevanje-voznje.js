...
const GET_PRODUCTS_BY_CATEGORY = gql`
  query ProductsByCategory {
    productsByCategory(category: "znaki za označevanje roba vozišča in preprečevanje vožnje" ){
      tag
      category
      subcategory
      class
...
class znakiZaOznacevanjeRobaVoziscaInPreprecevanjeVoznje extends React.Component {
  ...
  render() {
    ...
    return (
      <App pathname={"znaki-za-oznacevanje-roba-vozisca-in-preprecevanje-voznje"}>
        <Wrapper>
          <Query  query={GET_PRODUCTS_BY_CATEGORY}>
            {({ loading, error, data }) => {
              if(loading){
                return(
                  <React.Fragment>
                    <TitleH1>Znaki za označevanje roba vozisča in preprečevanje vožnje</TitleH1>
                    <SignsWrapper>
                      <Sign/><Sign/><Sign/><Sign/>
                      <Sign/><Sign/><Sign/><Sign/>
                      <Sign/><Sign/><Sign/><Sign/>
              ...
              return (
                <Signs subcategory2={subcategory} pathname={"znaki-za-oznacevanje-roba-vozisca-in-preprecevanje-voznje"} subcategories={subcategories} data={data} />
              );
            }}
          </Query>
        </Wrapper>
...
export default withApollo({ ssr: true })(withRouter(znakiZaOznacevanjeRobaVoziscaInPreprecevanjeVoznje))
