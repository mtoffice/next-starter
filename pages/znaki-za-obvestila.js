...
const GET_PRODUCTS_BY_CATEGORY = gql`
  query ProductsByCategory {
    productsByCategory(category: "znaki za obvestila" ){
      tag
      category
      subcategory
      class
...
class znakiZaObvestila extends React.Component {
  ...
  render() {
    ...
    return (
      <App pathname={"znaki-za-obvestila"}>
        <Wrapper>
          <Query  query={GET_PRODUCTS_BY_CATEGORY}>
            {({ loading, error, data }) => {
              if(loading){
                return(
                  <React.Fragment>
                    <TitleH1>Znaki za obvestila</TitleH1>
                    <SignsWrapper>
                      <Sign/><Sign/><Sign/><Sign/>
                      <Sign/><Sign/><Sign/><Sign/>
                      <Sign/><Sign/><Sign/><Sign/>
              ...
              return (
                <Signs subcategory2={subcategory} pathname={"znaki-za-obvestila"} subcategories={subcategories} data={data} />
              );
            }}
          </Query>
        </Wrapper>
...
export default withApollo({ ssr: true })(withRouter(znakiZaObvestila))
