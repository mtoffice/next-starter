...
const GET_PRODUCTS_BY_CATEGORY = gql`
  query ProductsByCategory {
    productsByCategory(category: "znaki za nevarnost" ){
      tag
      category
      subcategory
      class
...
class znakiZaNevarnost extends React.Component {
  ...
  render() {
    ...
    return (
      <App pathname={"znaki-za-nevarnost"}>
        <Wrapper>
          <Query  query={GET_PRODUCTS_BY_CATEGORY}>
            {({ loading, error, data }) => {
              if(loading){
                return(
                  <React.Fragment>
                    <TitleH1>Znaki za nevarnost</TitleH1>
                    <SignsWrapper>
                      <Sign/><Sign/><Sign/><Sign/>
                      <Sign/><Sign/><Sign/><Sign/>
                      <Sign/><Sign/><Sign/><Sign/>
              ...
              return (
                <Signs subcategory2={subcategory} pathname={"znaki-za-nevarnost"} subcategories={subcategories} data={data} />
              );
            }}
          </Query>
        </Wrapper>
