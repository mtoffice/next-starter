...
const GET_PRODUCTS_BY_CATEGORY = gql`
  query ProductsByCategory {
    productsByCategory(category: "znaki turistične in druge obvestilne signalizacije" ){
      tag
      category
      subcategory
      class
...
class znakiZaOznacevanjeRobaVoziscaInPreprecevanjeVoznje extends React.Component {
  ...
  render() {
    ...
    return (
      <App pathname={"znaki-turisticne-in-druge-obvestilne-signalizacije"}>
        <Wrapper>
          <Query  query={GET_PRODUCTS_BY_CATEGORY}>
            {({ loading, error, data }) => {
              if(loading){
                return(
                  <React.Fragment>
                    <TitleH1>Znaki turistične in druge obvestilne signalizacije</TitleH1>
                    <SignsWrapper>
                      <Sign/><Sign/><Sign/><Sign/>
                      <Sign/><Sign/><Sign/><Sign/>
                      <Sign/><Sign/><Sign/><Sign/>
              ...
              return (
                <Signs subcategory2={subcategory} pathname={"znaki-turisticne-in-druge-obvestilne-signalizacije"} subcategories={subcategories} data={data} />
              );
            }}
          </Query>
        </Wrapper>
